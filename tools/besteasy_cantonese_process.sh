...
do
    ...
    if [ -d $SUB_DIR ]; then
        ...
        # Step 2: run Paraformer finetuning cantonese ASR model on audios to generate txt format ASR result
        python cantonese_batch_inference.py --input_audio_path=$SUB_DIR/audio/ --output_path=$SUB_DIR/txt_result/ --device=$INFERENCE_DEVICE
